...
eth::OnOpFunc FakeExtVM::simpleTrace() const
{
    return [](uint64_t steps, uint64_t pc, eth::Instruction inst, bigint newMemSize, bigint gasCost,
        ...
        Array a_stack;
        for (auto i : vm.stack())
            a_stack.push_back(i.convert_to<string>());
        ...
        Object storage;
        for (auto const& i : std::get<2>(ext.addresses.find(ext.myAddress)->second))
            storage.push_back(Pair(i.first.convert_to<string>(), i.second.convert_to<string>()));

        /*add all the other details*/
        o_step.push_back(Pair("storage", storage));
        o_step.push_back(Pair("depth", to_string(ext.depth)));
        o_step.push_back(Pair("gas", gas.convert_to<string>()));
